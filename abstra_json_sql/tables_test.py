...
from .tables import (
    Column, ColumnType, Table, ForeignKey,
    InMemoryTables, FileSystemJsonTables, FileSystemJsonLTables, ExtendedTables
)
...
class TestFileSystemJsonTables:
    ...
    def test_add_column(self, tables, sample_table):
        tables.add_table(sample_table)
        new_column = Column(name="email", type=ColumnType.string, default="test@example.com")
        tables.add_column("users", new_column)
...
class TestExtendedTables:
    ...
    def test_get_table_from_base(self, base_tables, extra_table):
        base_table = Table(name="base", columns=[Column(name="id", type=ColumnType.int)], data=[])
    ...
    def test_add_table(self, base_tables, extra_table):
        extended = ExtendedTables(base_tables, [extra_table])
        new_table = Table(name="new", columns=[Column(name="id", type=ColumnType.int)], data=[])
