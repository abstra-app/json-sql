...
class ColumnType(Enum):
    ...
    def from_value(value: Any) -> "ColumnType":
        if isinstance(value, bool):  # Check bool first since bool is a subclass of int in Python
...
class ITablesSnapshot(ABC):
    ...
    @abstractmethod
    def change_column_type(self, table_name: str, column_name: str, new_type: ColumnType):
...
class FileSystemJsonTables(ITablesSnapshot):
    ...
    def _save_table_metadata(self, table_name: str, columns: List[Column]):
        ...
        for col in columns:
            ...
            # Ensure ColumnType is serialized as string
            if 'type' in col_dict and isinstance(col_dict['type'], ColumnType):
                col_dict['type'] = col_dict['type'].value
